...
router.get('/me', async (req, res) => {
  try {
    ...
    res.json({ user:{
      ...
      is_premium:isPrem, subscription_end:user.subscription_end,
      ...
      total_sessions:user.total_sessions||0, total_score:user.total_score||0
    }});
...
router.patch('/pseudo', async (req, res) => {
  try {
    ...
    if(!pseudo||pseudo.length<2||pseudo.length>30) return res.status(400).json({ error:'Pseudo entre 2 et 30 caractères.' });
    const db = getDatabase();
    const ex = await db.findOne('users',{pseudo});
    if(ex&&ex.id!==req.user.id) return res.status(409).json({error:'Pseudo déjà pris.'});
...
router.patch('/password', async (req, res) => {
  try {
    ...
    if(!old_password||!new_password||new_password.length<8) return res.status(400).json({ error:'Données invalides.' });
    const db = getDatabase();
    const user = await db.findOne('users',{id:req.user.id});
    if(!await bcrypt.compare(old_password,user.password_hash)) return res.status(401).json({error:'Ancien mot de passe incorrect.'});
...
// ============================================================
// NOTE: Pour les routes questions admin, créer:
// src/questions/questions.routes.js
// (Même logique que BobdoQuiz — POST /api/questions, POST /api/questions/bulk)
// avec middleware adminMiddleware
// ============================================================
