...
async function authMiddleware(req, res, next) {
  try {
    ...
    try { decoded = jwt.verify(token, process.env.JWT_SECRET); }
      ...
      if (e.name==='TokenExpiredError') return res.status(401).json({ error:'Token expirÃ©.', code:'TOKEN_EXPIRED' });
...
// Middleware: vÃ©rifier accÃ¨s quiz (free ou premium)
async function quizAccessMiddleware(req, res, next) {
  ...
  // Quota gratuit Ã©puisÃ© et pas de premium
  return res.status(402).json({
    ...
    plans: {
      ...
      premium_yearly:  { price: parseInt(process.env.PREMIUM_YEARLY_PRICE)||15000, label:'15 000 FCFA / an', duration:'1 an', savings:'Ã‰conomisez 21 000 FCFA vs mensuel' }
    }
  });
}
...
// ============================================================
// (inclus dans ce fichier pour rÃ©duire le nombre de fichiers)
...
router2.post('/start', async (req, res) => {
  ...
  try {
    ...
    if (activeSess[0]) return res.status(409).json({ error:'Session dÃ©jÃ  en cours.', session_id:activeSess[0].id });

    // SÃ©lectionner 10 questions jamais vues par cet user
    ...
    // IncrÃ©menter quiz_free_used si pas premium
    if (!req.isPremium) {
...
// POST /api/quiz/answer â€” VÃ©rification 100% cÃ´tÃ© serveur
router2.post('/answer', async (req, res) => {
  const userId = req.user.id;
  try {
    const { session_id, question_id, answer_index, time_taken_ms } = req.body;
    if (!session_id||question_id===undefined) return res.status(400).json({ error:'DonnÃ©es manquantes.' });
    ...
    if (!sess) return res.status(404).json({ error:'Session introuvable ou terminÃ©e.' });

    // VÃ©rifier HMAC (anti-tamper)
    ...
    if (alreadyAns[0]) return res.status(409).json({ error:'Question dÃ©jÃ  rÃ©pondue.' });

    // Charger la question avec la bonne rÃ©ponse (backend seulement)
    ...
    // Recalculer le shuffle (mÃªme seed = mÃªme ordre)
    ...
    // Mettre Ã  jour stats user si session terminÃ©e
    if (over) {
    ...
    // IncrÃ©menter compteurs question
    await db.query(
...
router2.get('/revisions', async (req, res) => {
  ...
  } catch(e) { res.status(500).json({ error:'Erreur rÃ©visions.' }); }
});
